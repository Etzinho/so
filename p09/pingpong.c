...
void dispatcher_body(){
	...
	task_t *aux = queue_t;
	while(1){
		if(aux->id == 1 && aux->next->id == 1 && sleep_t == NULL){task_exit(0);}
		schedu = scheduler();
		if(schedu != NULL){
			task_switch(schedu);
		}
...
void wake_tasks(int exit_code, int id){
	...
	if(*nova != NULL){
		do{
			...
			else if((*nova)->sleep <= systime() && (*nova)->sleep > 0){
				(*nova)->sleep = 0;
				task_resume(*nova);
			}
		nova = &((*nova)->next);}while((*nova)->id != sleep_t->id);
	}
}

void  task_sleep(int t){
	task_running->sleep = systime() + (t*1000);
	task_suspend(task_running, &sleep_t);
}
